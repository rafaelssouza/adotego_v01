...
-- -----------------------------------------------------
-- Table `adotego`.`Doacao`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `adotego`.`Doacao` (
  `idDoacao` INT NOT NULL AUTO_INCREMENT,
  `valor` DOUBLE NOT NULL,
  `doacao_idPessoa` INT NULL,
  PRIMARY KEY (`idDoacao`),
  INDEX `fk_Doacao_Pessoa1_idx` (`doacao_idPessoa` ASC),
  CONSTRAINT `fk_Doacao_Pessoa1`
    FOREIGN KEY (`doacao_idPessoa`)
    REFERENCES `adotego`.`Pessoa` (`idPessoa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Data for table `adotego`.`Situacao`
-- -----------------------------------------------------
START TRANSACTION;
USE `adotego`;
INSERT INTO `adotego`.`Situacao` (`idSituacao`, `descricao`) VALUES (1, 'Disponivel');
INSERT INTO `adotego`.`Situacao` (`idSituacao`, `descricao`) VALUES (2, 'Não Disponível');
...
alter table doacao add column data_doacao timestamp default current_timestamp();
