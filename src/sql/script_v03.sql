...
CREATE TABLE IF NOT EXISTS `adotego`.`Adocao` (
  ...
  INDEX `fk_Adocao_Doacao1_idx` (`adocao_idDoacao` ASC),
  ...
  CONSTRAINT `fk_Adocao_Animal1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Adocao_Doacao1`
    FOREIGN KEY (`adocao_idDoacao`)
    REFERENCES `adotego`.`Doacao` (`idDoacao`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
